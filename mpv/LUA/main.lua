...
function M._ensure_cursor_autohide_default()
    local ok, current = pcall(mp.get_property, 'cursor-autohide')
    if not (ok and tostring(current or '') == 'no') then
...
local function _run_helper_request_async(req, timeout_seconds, cb)
    ...
    local function done(resp, err)
        ...
        local is_retryable = is_timeout and type(req) == 'table' and retry_count < 1
            and (op_name == 'ytdlp-formats' or op_name == 'ytdlp-resolve' or op_name == 'run-background')
...
local function _is_direct_media_url(u)
    local low = tostring(u or ''):lower()
    if low == '' then
        return false
    end
...
mp.register_script_message('medios-change-format-current', function()
    ...
    if _is_direct_media_url(tostring(target)) then
        ...
        if page_url and page_url ~= '' then
            ...
            mp.osd_message('No page URL for this stream; load the YouTube URL again', 4)
            return
        end
    end
...
mp.register_script_message('medios-load-url-event', function(json)
    ...
    if _is_ytdlp_url(url) then
        _lua_log('[LOAD-URL] yt-dlp URL; resolving via ytdlp plugin')
        mp.osd_message('Resolving URL via yt-dlp...', 3)
        ensure_mpv_ipc_server()
